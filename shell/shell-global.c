...
{
  const char *datadir = g_getenv ("DAWATI_SHELL_DATADIR");
  const char *shell_js = g_getenv("DAWATI_SHELL_JS");
  char *imagedir, **search_path;

  if (!datadir)
    datadir = DAWATI_SHELL_DATADIR;
  ...
  global->settings = g_settings_new ("org.dawati.shell");
  ...
  ca_context_change_props (global->sound_context, CA_PROP_APPLICATION_NAME, PACKAGE_NAME, CA_PROP_APPLICATION_ID, "org.dawati.Shell", NULL);
