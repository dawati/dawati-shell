  ...
  for (iter = priv->launchers; iter; iter = iter->next)
    {
      ...
      if (g_list_find_custom (apps, exec, (GCompareFunc) g_strcmp0) != NULL)
        {
          g_object_ref (button);
          clutter_actor_remove_child (priv->apps_grid, CLUTTER_ACTOR (button));
          clutter_actor_add_child (priv->apps_grid, CLUTTER_ACTOR (button));
          clutter_actor_show (CLUTTER_ACTOR (button));
                             ...
                             MnbLauncher  *self)
{
  ...
  label = mx_button_get_label (MX_BUTTON (button));
  toggled_on = mx_button_get_toggled (MX_BUTTON (button));


  if (g_strcmp0 (label, "fav") == 0)
    {
      mnb_launcher_show_favourites (self, toggled_on);
      return;
    }
#ifdef WITH_ZEITGEIST
  if (g_strcmp0 (label, "most") == 0 || g_strcmp0 (label, "recent") == 0)
    {
      mnb_launcher_show_zg_category(self, toggled_on, label);
      return;
    }
#endif /* WITH_ZEITGEIST */

  if (g_strcmp0 (label, "current") == 0)
    {
  ...
  if (g_strcmp0 (label, "all") == 0)
    {
      mnb_launcher_filter_cb (self);
      return;
    }

  needle = g_strdup (label);
  needle = g_strstrip (needle);
...
mnb_launcher_category_button_new (MnbLauncher *self, const gchar *text)
{
  ...
  if (g_strcmp0 (text, "recent") == 0)
    text = _("Recently Used");
#endif /* WITH_ZEITGEIST */
    ...
    if (n_buttons > 0)
      {
        ...
        clutter_actor_add_child (priv->category_section, category);
      }
  return TRUE;
}
...
{
  clutter_actor_add_child (CLUTTER_ACTOR (apps_grid),
